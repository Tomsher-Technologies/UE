...
class RateExport implements FromCollection, WithHeadings
{
    ...
    public function collection()
    {
        if ($this->request->export_by == 'integrator') {
            ...
            $data = $model::with('zone')->where('integrator_id', $this->request->integrator)->get();
            $this->unique_weight = $data->pluck('weight')->unique();
            ...
            foreach ($this->unique_weight as $weight) {
                ...
                foreach ($this->zone_unique as  $zone) {
                    $array[$zone]  = $data->where('zone.zone_code', $zone)->where('weight', $weight)->pluck('rate')->first() ?? 0;
                }
