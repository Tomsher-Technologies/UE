    ...
    @if (isset($actions) && count($actions) && $row !== '')
        @foreach ($actions as $key => $action)
            <td class="pg-actions {{ $theme->table->tdBodyClass }}" style="{{ $theme->table->tdBodyStyle }}">
                ...
                @endphp
                <div class="w-full md:w-auto" style="display: {{ $ruleHide ? 'none' : 'block' }}">
                    @if ((filled($action->event) || isset($event['event']) || filled($action->view)) && is_null($ruleRedirect) && $action->event !== '')
                        <button
                            @if ($action->emit) wire:click='$emit("{{ $event['event'] }}", @json($event['params']))' @endif
                            @if ($action->emitTo) wire:click='$emitTo("{{ $event['to'] }}", "{{ $event['event'] }}", @json($event['params']))' @endif
                            @if ($action->view) wire:click='$emit("openModal", "{{ $action->view }}", @json($actionParameters))' @endif
                            @if ($ruleDisabled) disabled @endif title="{{ $action->tooltip }}"
                            {{ $class }}>
                    ...
                    @else
                       ...
                       @else
                       href="{{ $ruleRedirect['url'] }}" target="{{ $ruleRedirect['target'] }}" @endif
                                title="{{ $action->tooltip }}" {{ $class }}>
                                {!! $ruleCaption ?? $action->caption !!}
                            </a>
                        @endif
                        ...
                        @if (filled($action->route))
                            @if (strtolower($action->method) !== 'get')
                                <form target="{{ $action->target }}"
                                    action="{{ route($action->route, $actionParameters) }}" method="post">
                            ...
                            @else
                                <a href="{{ route($action->route, $actionParameters) }}"
                                    target="{{ $action->target }}" title="{{ $action->tooltip }}"
                                    {{ $class }}>
                                    {!! $ruleCaption ?? $action->caption !!}
                                </a>
                            @endif
