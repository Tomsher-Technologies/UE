    ...
    <div class="container page__container">
        <div class="page-section">
            <div class="page-separator">
                <div class="page-separator__text">Upload File</div>
            </div>
            ...
            @if (session('import_errors') && count(session('import_errors')) > 0)
                <div class="alert alert-danger">
                    ...
                    besause the {{ Str::plural('zone', count(session('import_errors'))) }} does not exist. Please create the
                    {{ Str::plural('zone', count(session('import_errors'))) }} first and then try uploading again.
                </div>
            @endif

            <form method="POST" action="{{ route('admin.integrator.upload', $integrator) }}" enctype="multipart/form-data">
                @csrf
